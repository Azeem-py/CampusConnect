...
export function ProfilePage() {
  ...
  const { data: postsData, isLoading: postsLoading } = useUserPosts(
    activeTab === "saved" ? undefined : user?.id,
    1,
    20,
    activeTab === "saved" ? user?.id : undefined
  )
  ...
  const interests = user.interests?.split(",").map((s) => s.trim()).filter(Boolean) ?? []
        ...
        <main className="flex-1 max-w-[600px] min-w-0 space-y-4">
          <div className="bg-surface-container-lowest border border-outline-variant/15 rounded-lg overflow-hidden">
            ...
            <div className="px-4 pb-4">
              ...
              <div>
                <h1 className="font-geist font-bold text-display-md text-on-surface">{user.name}</h1>
                <p className="text-body-md text-on-surface-variant font-inter">{user.username}</p>
              </div>

              <div className="flex items-center gap-2 mt-2">
                {user.department && <Tag variant="department">{user.department}</Tag>}
                {user.major && <Tag variant="skill">{user.major}</Tag>}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-3 text-body-sm text-on-surface-variant font-inter">
                {user.school && (
                  <span className="flex items-center gap-1">
                    <MapPin size={13} /> {user.school}
                  </span>
                )}
                <span className="flex items-center gap-1">
                  <Calendar size={13} /> Joined {formatJoinDate(user.createdAt)}
                </span>
              </div>

              <div className="flex items-center gap-4 mt-4 text-title-md font-geist">
                <span>
                  <strong className="text-on-surface">{user.reputationScore}</strong>{" "}
          ...
          {user.school && (
            <div className="bg-surface-container-lowest border border-outline-variant/15 rounded-lg p-4 space-y-4">
              <div>
                ...
                <p className="mt-1 text-title-sm text-on-surface font-geist font-medium">{user.school}</p>
                {user.department && <p className="text-body-sm text-on-surface-variant font-inter">{user.department}</p>}
                {user.major && <p className="text-body-sm text-on-surface-variant font-inter">Major: {user.major}</p>}
              </div>
            </div>
          )}
